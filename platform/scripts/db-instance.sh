...
sudo incus exec db1 -- sudo adduser --system --shell /bin/bash --home /home/sportsoil sportsoil
sudo incus exec db1 -- sudo -u postgres createuser -d sportsoil
sudo incus exec db1 -- sudo -u sportsoil createdb -O sportsoil sportsoil

# add pg_hba entry for client and accept tcp connections
sudo incus exec db1 -- sudo -u postgres sed -r -i.BAK "/#listen_addresses/a\listen_addresses = '*'" /etc/postgresql/15/main/postgresql.conf
...
sudo incus exec db1 -- sudo -u postgres cp /etc/postgresql/15/main/pg_hba.conf /etc/postgresql/15/main/pg_hba.conf.bak
sudo incus exec db1 -- sudo -u postgres sh -c 'cat <<EOF >> /etc/postgresql/15/main/pg_hba.conf
